...
extern "C" LLVMTargetMachineRef LLVMRustCreateTargetMachine(
    ...
    bool DataSections, bool TrapUnreachable, bool Singlethread,
    bool AsmComments) {
...
// // Unfortunately, LLVM doesn't expose a C API to add the corresponding
// analysis
// // passes for a target to a pass manager. We export that functionality
// through
// // this function.
...
// // Unfortunately, the LLVM C API doesn't provide a way to set the
// `LibraryInfo`
...
// extern "C" void LLVMRustAddLibraryInfo(LLVMPassManagerRef PMR, LLVMModuleRef
// M,
//                                        bool DisableSimplifyLibCalls)
// {
//   Triple TargetTriple(unwrap(M)->getTargetTriple());
...
// // Unfortunately, the LLVM C API doesn't provide an easy way of iterating
// over
...
//   for (Module::iterator I = unwrap(M)->begin(), E = unwrap(M)->end(); I !=
//   E;)
...
//   // Apparently `addPassesToEmitFile` adds a pointer to our on-the-stack
//   output
...
//         // realistically never will (and even if, the result is
//         implementation
...
// {
//   for (Module::iterator GV = unwrap(M)->begin(), E = unwrap(M)->end(); GV !=
//   E;
//        ++GV)
//   {
//     GV->setDoesNotThrow();
...
// extern "C" bool LLVMRustThinLTOAvailable()
// {
// #if LLVM_VERSION_GE(4, 0)
//   return true;
// #else
...
// extern "C" bool LLVMRustPGOAvailable()
// {
// #ifdef PGO_AVAILABLE
//   return true;
// #else
...
// // right now. This ThinLTO support is only enabled on "recent ish" versions
// of
...
// // sending this upstream to LLVM. I hear though they're quite eager to
// receive
// // feedback like this!
// //
// // If you're reading this code and wondering "what in the world" or you're
// // working "good lord by LLVM upgrade is *still* failing due to these
// bindings"
...
// // here. Not saying I do myself, but it took me awhile staring at LLVM's
// source
// // and various online resources about ThinLTO to make heads or tails of all
// // this.

// // This is a shared data structure which *must* be threadsafe to share
// // read-only amongst threads. This also corresponds basically to the
// arguments
...
//   // All modules we may look at, stored as in-memory serialized versions.
//   This
//   // is later used when inlining to ensure we can extract any module to
//   inline
//   // from.
//   StringMap<MemoryBufferRef> ModuleMap;

//   // A set that we manage of everything we *don't* want internalized. Note
//   that
//   // this includes all transitive references right now as well, but it may
//   not
//   // always!
...
// //   Summary) Ret->Index.collectDefinedGVSummariesPerModule(
// //       Ret->ModuleToDefinedGVSummaries);

// //   // Convert the preserved symbols set from string to GUID, this is then
// //   needed
// //   // for internalization.
...
// //   // Collect the import/export lists for all modules from the call-graph
// in
// //   the
// //   // combined index
// //   //
...
// //   Ret->GUIDPreservedSymbols); ComputeCrossModuleImport(Ret->Index,
// //   Ret->ModuleToDefinedGVSummaries,
// //                            Ret->ImportLists, Ret->ExportLists,
// &DeadSymbols);
// // #endif
...
// //   // impacts the caching.
// //   //
// //   // This is copied from `lib/LTO/ThinLTOCodeGenerator.cpp` with some of
// this
// //   // being lifted from `lib/LTO/LTO.cpp` as well
// //   StringMap<std::map<GlobalValue::GUID, GlobalValue::LinkageTypes>>
// //   ResolvedODR; DenseMap<GlobalValue::GUID, const GlobalValueSummary *>
// //   PrevailingCopy; for (auto &I : Ret->Index) {
// // #if LLVM_VERSION_GE(5, 0)
...
// that
// //   dead
// //   // symbols are internalized. Otherwise everything that's already
// external
...
// // with one another, one per module. The passes here correspond to the
// analysis
...
//   const auto &ImportList =
//   Data->ImportLists.lookup(Mod.getModuleIdentifier()); auto Loader =
...
//     // https://bugs.llvm.org/show_bug.cgi?id=38184 where during ThinLTO
//     imports
...
//     // `wasm.custom_sections` named metadata node from any imported module.
//     This
//     // we know isn't used by any optimization pass so there's no need for it
//     to
//     // be imported.
//     //
//     // Note that the metadata is currently lazily loaded, so we materialize
//     it
//     // here before looking up if there's metadata inside. The
//     `FunctionImporter`
...
//                                 module_name_callback, void *callback_payload)
// {
...
// extern "C" LLVMRustThinLTOBuffer *LLVMRustThinLTOBufferCreate(LLVMModuleRef
// M)
// {
...
// // processing.  We'll call this once per module optimized through ThinLTO,
// and
// // it'll be called concurrently on many threads.
// extern "C" LLVMModuleRef
// LLVMRustParseBitcodeForThinLTO(LLVMContextRef Context, const char *data,
//                                size_t len, const char *identifier)
// {
...
//   // merge all the existing compile units. If there aren't actually any
//   though
...
//   // process it recursively. Note that we specifically iterate over
//   instructions
...
//   // Erase any other references to other `DICompileUnit` instances, the
//   verifier
//   // will later ensure that we don't actually have any other stale references
//   to
//   // worry about.
