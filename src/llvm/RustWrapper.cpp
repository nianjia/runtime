...
//   return wrap(
//       unwrap(M)->getOrInsertFunction(Name,
...
// extern "C" void LLVMRustAddCallSiteAttribute(LLVMValueRef Instr, unsigned
// Index,
...
//   Call.setAttributes(
//       Call.getAttributes().addAttributes(Call->getContext(), Index, B));
// }
...
// extern "C" void LLVMRustAddDereferenceableOrNullCallSiteAttr(LLVMValueRef
// Instr,
//                                                              unsigned Index,
//                                                              uint64_t Bytes)
//                                                              {
...
// extern "C" void LLVMRustAddAlignmentAttr(LLVMValueRef Fn, unsigned Index,
//                                          uint32_t Bytes) {
//   Function *A = unwrap<Function>(Fn);
//   AttrBuilder B;
//   B.addAlignmentAttr(Bytes);
...
// extern "C" void LLVMRustAddDereferenceableAttr(LLVMValueRef Fn, unsigned
// Index,
//                                                uint64_t Bytes) {
//   Function *A = unwrap<Function>(Fn);
//   AttrBuilder B;
...
// extern "C" LLVMValueRef LLVMRustMetadataAsValue(LLVMContextRef C,
//                                                 LLVMMetadataRef MD) {
...
// LLVMRustDIBuilderCreateFile(LLVMRustDIBuilderRef Builder, const char
// *Filename,
...
// extern "C" LLVMMetadataRef
// LLVMRustDIBuilderCreateBasicType(LLVMRustDIBuilderRef Builder, const char
// *Name,
...
//     uint32_t AlignInBits, LLVMRustDIFlags Flags, LLVMMetadataRef DerivedFrom,
...
//     LLVMRustDIBuilderRef Builder, LLVMMetadataRef Scope, const char *Name,
//     LLVMMetadataRef File, unsigned LineNumber, uint64_t SizeInBits,
//     uint32_t AlignInBits, LLVMRustDIFlags Flags, LLVMMetadataRef
//     Discriminator, LLVMMetadataRef Elements, const char *UniqueId) {
// #if LLVM_VERSION_GE(7, 0)
//   return wrap(Builder->createVariantPart(
//       unwrapDI<DIDescriptor>(Scope), Name, unwrapDI<DIFile>(File),
//       LineNumber, SizeInBits, AlignInBits, fromRust(Flags),
//       unwrapDI<DIDerivedType>(Discriminator),
...
//       SizeInBits, AlignInBits, OffsetInBits, D, fromRust(Flags),
//       unwrapDI<DIType>(Ty)));
// #else
//   return wrap(Builder->createMemberType(unwrapDI<DIDescriptor>(Scope), Name,
//                                         unwrapDI<DIFile>(File), LineNo,
...
//                                     LLVMMetadataRef Scope, LLVMMetadataRef
//                                     File, unsigned Line, unsigned Col) {
...
//     LLVMRustDIBuilderRef Builder, LLVMMetadataRef Scope, LLVMMetadataRef
//     File) {
...
//           IsLocalToUnit, InitExpr, unwrapDIPtr<MDNode>(Decl),
...
//     const char *Name, LLVMMetadataRef File, unsigned LineNo, LLVMMetadataRef
//     Ty, bool AlwaysPreserve, LLVMRustDIFlags Flags, unsigned ArgNo, uint32_t
//     AlignInBits) {
...
// extern "C" LLVMMetadataRef
// LLVMRustDIBuilderGetOrCreateSubrange(LLVMRustDIBuilderRef Builder, int64_t
// Lo,
//                                      int64_t Count) {
...
//   return wrap(
//       Builder->getOrCreateArray(ArrayRef<Metadata *>(DataValue,
//       Count)).get());
// }
...
//       Builder->createExpression(llvm::ArrayRef<int64_t>(AddrOps,
//       AddrOpsCount)),
...
// extern "C" LLVMValueRef
// LLVMRustDIBuilderCreateDebugLocation(LLVMContextRef ContextRef, unsigned
// Line,
//                                      unsigned Column, LLVMMetadataRef Scope,
//                                      LLVMMetadataRef InlinedAt) {
//   LLVMContext &Context = *unwrap(ContextRef);

//   DebugLoc debug_loc = DebugLoc::get(Line, Column,
...
// // function provided by LLVM doesn't return the length, so we've created our
// own
// // function which returns the length as well as the data pointer.
// //
// // For an example of this not returning a null terminated string, see
// // lib/Object/COFFObjectFile.cpp in the getSectionName function. One of the
// // branches explicitly creates a StringRef without a null terminator, and
// then
// // that's returned.
...
// extern "C" void
// LLVMRustUnpackInlineAsmDiagnostic(LLVMDiagnosticInfoRef DI, unsigned
// *CookieOut,
...
// {
//   return wrap(
//       unwrap(B)->CreateCatchRet(cast<CatchPadInst>(unwrap(Pad)),
//       unwrap(BB)));
// }
...
// extern "C" LLVMValueRef LLVMRustBuildMemCpy(LLVMBuilderRef B, LLVMValueRef
// Dst,
//                                             unsigned DstAlign, LLVMValueRef
//                                             Src, unsigned SrcAlign,
//                                             LLVMValueRef Size,
//                                             bool IsVolatile)
// {
// #if LLVM_VERSION_GE(7, 0)
//   return wrap(unwrap(B)->CreateMemCpy(unwrap(Dst), DstAlign, unwrap(Src),
//                                       SrcAlign, unwrap(Size), IsVolatile));
// #else
//   unsigned Align = std::min(DstAlign, SrcAlign);
//   return wrap(unwrap(B)->CreateMemCpy(unwrap(Dst), unwrap(Src), unwrap(Size),
//                                       Align, IsVolatile));
// #endif
// }
...
// {
// #if LLVM_VERSION_GE(7, 0)
//   return wrap(unwrap(B)->CreateMemMove(unwrap(Dst), DstAlign, unwrap(Src),
//                                        SrcAlign, unwrap(Size), IsVolatile));
// #else
//   unsigned Align = std::min(DstAlign, SrcAlign);
//   return wrap(unwrap(B)->CreateMemMove(unwrap(Dst), unwrap(Src),
//   unwrap(Size),
//                                        Align, IsVolatile));
// #endif
// }
...
//   unsigned Len = Bundle ? 1 : 0;
//   ArrayRef<OperandBundleDef> Bundles = makeArrayRef(Bundle, Len);
//   return wrap(unwrap(B)->CreateInvoke(unwrap(Fn), unwrap(Then),
//   unwrap(Catch),
...
// extern "C" LLVMValueRef LLVMRustBuildIntCast(LLVMBuilderRef B, LLVMValueRef
// Val,
//                                              LLVMTypeRef DestTy,
//                                              bool isSigned)
// {
...
// #if LLVM_VERSION_GE(6, 0)
// extern "C" LLVMValueRef LLVMRustBuildMinNum(LLVMBuilderRef B, LLVMValueRef
// LHS,
//                                             LLVMValueRef RHS)
// {
//   return wrap(unwrap(B)->CreateMinNum(unwrap(LHS), unwrap(RHS)));
// }
// extern "C" LLVMValueRef LLVMRustBuildMaxNum(LLVMBuilderRef B, LLVMValueRef
// LHS,
//                                             LLVMValueRef RHS)
// {
...
// }
// #else
// extern "C" LLVMValueRef LLVMRustBuildMinNum(LLVMBuilderRef, LLVMValueRef,
//                                             LLVMValueRef)
// {
//   return nullptr;
// }
// extern "C" LLVMValueRef LLVMRustBuildMaxNum(LLVMBuilderRef, LLVMValueRef,
//                                             LLVMValueRef)
// {
//   return nullptr;
// }
...
LLVMRustCreateTargetMachine(const char *TripleStr, const char *CPU,
                            const char *Feature, LLVMRustCodeModel RustCM,
                            LLVMRustRelocMode RustReloc,
                            LLVMRustCodeGenOptLevel RustOptLevel)
{
  ...
  TargetMachine *TM = TheTarget->createTargetMachine(
      Trip.getTriple(), CPU, Feature, Options, RM, None, OptLevel);
  return wrap(TM);
}
...
extern "C" bool LLVMRustInitializeNativeTargetAsmParser()
{
  return llvm::InitializeNativeTargetAsmParser();
}

extern "C" bool LLVMRustInitializeNativeTargetDisassembler()
{
  return llvm::InitializeNativeTargetDisassembler();
}

extern "C" LLVMMemoryBufferRef LLVMRustTargetMachineEmitToMemoryBuffer(LLVMTargetMachineRef T, LLVMModuleRef M, LLVMCodeGenFileType codegen, char **ErrorMessage)
{
  LLVMMemoryBufferRef OutMemBuf = NULL;
  if (LLVMTargetMachineEmitToMemoryBuffer(T, M, codegen, ErrorMessage, &OutMemBuf))
    return NULL;
  else
    return OutMemBuf;
}